...
<?php do_action( 'dokan_settings_before_form', $current_user, $profile_info ); ?>
    ...
    <form method="post" id="store-form"  action="" class="dokan-form-horizontal">
        ...
        <div class="dokan-form-group">
            ...
            <div class="dokan-w5 dokan-text-left">
                <input id="setting_phone" value="<?php echo esc_attr( $phone ); ?>" name="setting_phone" placeholder="<?php esc_attr_e( '+123456..', 'dokan-lite' ); ?>" class="form-control input-md" type="text">
            </div>
        </div>
            ...
            <div class="dokan-w6" style="width: auto">
                <?php foreach ( $dokan_days as $key => $day ) : ?>
                    <div class="dokan-form-group">
                        ...
                        <label for="">
                            <select name="<?php echo esc_attr( $day ) ?>_on_off" class="dokan-on-off form-control">
                                <option value="close" <?php  (!empty( $all_times ) and isset($all_times[$day]['open'])) ? selected( $all_times[$day]['open'], 'close' ) : '' ?> >
                                    <?php esc_html_e( 'Close', 'dokan-lite' ); ?>
                                </option>
                                <option value="open" <?php  (!empty( $all_times ) and isset($all_times[$day]['open'])) ? selected( $all_times[$day]['open'], 'open' ) : '' ?> >
        ...
        var dokan_address_select = {
            ...
            state_select: function () {
                var states_json = wc_country_select_params.countries.replace( /&quot;/g, '"' ),
                    ...
                    input_class = $statebox.attr( 'class' )+' form-control',
                    value = $statebox.val(),
                ...
                if ( states[ country ] ) {
                    ...
                    if ( $.isEmptyObject( states[ country ] ) ) {
                        ...
                        if ( $statebox.is( 'input' ) ) {
                            $( 'input#dokan_address_state' ).replaceWith( '<select type="text" class="' + input_class + ' form-control" name="' + input_name + '" id="' + input_id + '" required ></select>' );
                            $( 'select#dokan_address_state' ).html( '<option value="">' + wc_country_select_params.i18n_select_state_text + '</option>' + options );
                        }
